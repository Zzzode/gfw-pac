...
def main():
    ...
    if (args.input):
        ...
        content = urllib.request.urlopen(gfwlist_url,
    ...
    if args.user_rule:
        ...
        if not userrule_parts.scheme or not userrule_parts.netloc:
            ...
            user_rule = urllib.request.urlopen(
                args.user_rule, timeout=10).read().decode('utf-8')
    ...
    if args.direct_rule:
        ...
        if not directrule_parts.scheme or not directrule_parts.netloc:
            ...
            # Yeah, it's an URL, try to download it
            print('Downloading user rules file from %s' % args.user_rule)
            direct_rule = urllib.request.urlopen(
                args.direct_rule, timeout=10).read().decode('utf-8')
    ...
    if args.localtld_rule:
        ...
        if not tldrule_parts.scheme or not tldrule_parts.netloc:
            ...
            localtld_rule = urllib.request.urlopen(
                args.localtld_rule, timeout=10).read().decode('utf-8')
